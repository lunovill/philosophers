...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/24 21:49:30 by lunovill          #+#    #+#             */
/*   Updated: 2022/09/24 21:49:31 by lunovill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (args[i])
	{
		...
		while (args[i][j])
			if (ft_strichr("0123456789", args[i][j++]) == -1)
				return (ft_error("chk_args", "invalid charactere(s)",
						tbl_free, NULL));
		i++;
	}
	return (0);
...
static int	chk_overflow(char **args)
{
	...
	while (args[++i])
	{
		...
		if (len > 10)
			return (ft_error("chk_overflow",
					"number(s) is too big. Max is 4294967294", tbl_free, NULL));
...
int	chk_all(char **args)
{
	...
	if (error)
		...
		return (ft_error("chk_all", "numbre of philosophers is null",
				tbl_free, NULL),
